...
output "test_connectivity_command" {
  ...
  value       = <<-EOT
    ...
    1. SSH into Primary instance: ssh -i ${var.primary_key_name}.pem ubuntu@${aws_instance.primary_instance.public_ip}
    2. Ping Secondary instance: ping ${aws_instance.secondary_instance.private_ip}
    
    Or:
    1. SSH into Secondary instance: ssh -i ${var.secondary_key_name}.pem ubuntu@${aws_instance.secondary_instance.public_ip}
